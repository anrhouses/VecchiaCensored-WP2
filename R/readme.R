...
model_freq_no_cen=fit_model(grille_GP$Z,locs_obs = grille_GP[,1:2],X_obs = grille_GP[,c("X1","X2")],M = 30,censored_indices = rep(0,length(grille_GP$Z)))
summary.fit_model(model_freq_no_cen) # print parameters estimation result

prediction=prediction(model_freq_no_cen,locs_pred = test_GP[,1:2],X_pred = test_GP[,c("X1","X2")],M = 30) # perform prediction
...
## censuré
model_freq_cen=fit_model(Y_obs = grille_GP$censored,locs_obs = grille_GP[,1:2],X_obs = grille_GP[,c("X1","X2")],M = 30,censored_indices = grille_GP$is_censored)
summary.fit_model(model_freq_cen)  # print parameters estimation result


prediction=prediction(model_freq_cen,locs_pred = test_GP[,1:2],X_pred = test_GP[,c("X1","X2")],M = 30)# perform prediction
...
## no censuré
model_bay_no_cen=fit_model(grille_GP$Z,locs_obs = grille_GP[,1:2],X_obs = grille_GP[,c("X1","X2")],M = 10,censored_indices = rep(0,length(grille_GP$Z)),bayesian = TRUE,chains = 3,iter_warmup = 100,iter_sampling = 300,parallel_chains = 3)
...
prediction=prediction(model_bay_no_cen,locs_pred = test_GP[,1:2],X_pred = test_GP[,c("X1","X2")],M = 30)
plot(test_GP$Z,prediction$prediction)
cor(test_GP$Z,prediction$prediction) # same result as frequentist

## censored

model_bay_cen=fit_model(grille_GP$censored,locs_obs = grille_GP[,1:2],X_obs = grille_GP[,c("X1","X2")],M = 10,censored_indices = grille_GP$is_censored,bayesian = TRUE,chains = 3,iter_warmup = 100,iter_sampling = 300,parallel_chains = 3)

summary.fit_model(model_bay_no_cen)


prediction=prediction(model_bay_cen,locs_pred = test_GP[,1:2],X_pred = test_GP[,c("X1","X2")],M = 30)
plot(test_GP$Z,prediction$prediction)
...
model_freq_no_cen=fit_model(grille_SVC$Z,locs_obs = grille_SVC[,1:2],X_obs = grille_SVC[,c("X1","X2")],M = 30,censored_indices = rep(0,length(grille_SVC$Z)),svc_indices = 1:2)
summary.fit_model(model_freq_no_cen) # print parameters estimation result

prediction=prediction(model_freq_no_cen,locs_pred = test_SVC[,1:2],X_pred = test_SVC[,c("X1","X2")],M = 30) # perform prediction
...
model_bay_cen=fit_model(grille_SVC$censored,locs_obs = grille_SVC[,1:2],X_obs = grille_SVC[,c("X1","X2")],M = 10,censored_indices = grille_SVC$is_censored,bayesian = TRUE,chains = 3,iter_warmup = 100,iter_sampling = 300,parallel_chains = 3,svc_indices = c(1,2))

summary.fit_model(model_bay_cen)

# prediction with simuations and pred of coefitions !
prediction=prediction(model_bay_cen,locs_pred = test_SVC[,1:2],X_pred = test_SVC[,c("X1","X2")],M = 30,simulations = TRUE,pred_coef = TRUE)
